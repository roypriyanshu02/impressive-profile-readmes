...
async function fetchRepoStats() {
	const defaultStats = { stars: 0, open_issues: 0, open_prs: 0, forks: 0 };
	...
	try {
		...
		if (!repoRes || !repoRes.ok) return defaultStats;
		...
		return {
			...
			forks: repoData.forks_count ?? defaultStats.forks
		};
	} catch (err) {
		log('warn', `Failed to fetch repo stats: ${err.message}`);
		return defaultStats;
	}
}
